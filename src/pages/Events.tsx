          ...
          <div className="mx-auto max-w-[1320px] px-4 pt-6 sm:px-6 sm:pt-8 md:px-10 md:pt-10">
            <Link to={`/events/${featured.slug || featured.id}`} className="card-lift group relative block overflow-hidden rounded-[14px] sm:rounded-[18px]" style={{ border: `1px solid ${FJ.rule}` }}>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 md:p-8">
                ...
                <h2 className="mt-2 max-w-3xl font-serif text-[18px] font-extrabold leading-snug text-white sm:text-[22px] md:text-[28px]">{featured.title}</h2>
                <p className="mt-1 hidden max-w-2xl font-sans text-[13px] leading-relaxed text-white/80 sm:line-clamp-1 md:line-clamp-2">{excerpt(featured.content, 160)}…</p>
                <div className="mt-2 flex flex-wrap items-center gap-2 sm:mt-3 sm:gap-3">
                  <span className="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 font-sans text-[11px] font-semibold sm:px-4 sm:py-2 sm:text-[13px]" style={{ background: '#fff', color: FJ.ink }}>
          ...
          <div className="flex flex-col gap-3 rounded-[14px] p-3 sm:p-4 lg:flex-row lg:items-center" style={{ background: FJ.bg, border: `1px solid ${FJ.rule}` }}>
              ...
              {[{ id: 'all', name: t('events.catAll') }, ...topLevelCats.map((c) => ({ id: c.name, name: c.name }))].map((c) => {
                ...
                return (
                  <button key={c.id} type="button" onClick={() => reset(() => setFilter(c.id))}
                    className="rounded-full px-3 py-1 font-sans text-[11.5px] font-semibold transition-all sm:px-3.5 sm:py-1.5 sm:text-[12.5px]"
            ...
            <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-center sm:gap-2.5">
              <select value={sortOrder} onChange={(e) => reset(() => setSortOrder(e.target.value as 'newest' | 'oldest'))}
                className="rounded-[8px] px-2.5 py-2 font-sans text-[12px] outline-none sm:px-3 sm:text-[13px]" style={{ background: FJ.paper, color: FJ.ink2, border: `1px solid ${FJ.rule}` }}>
              ...
              <div className="relative col-span-2 sm:col-span-1">
                ...
                <input value={query} onChange={(e) => reset(() => setQuery(e.target.value))} placeholder={tr('Search events…', 'অনুষ্ঠান খুঁজুন…')}
                  className="w-full rounded-[8px] py-2 pl-9 pr-3 font-sans text-[12px] outline-none sm:text-[13px] lg:w-52" style={{ background: FJ.paper, color: FJ.ink, border: `1px solid ${FJ.rule}` }} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-[1320px] px-6 pb-16 pt-8 md:px-10">
          {loading ? (
            ...
            <div className="rounded-[14px] py-20 text-center" style={{ border: `1px dashed ${FJ.rule}` }}>
              <p className="font-sans text-[16px]" style={{ color: FJ.ink }}>{tr('No events match your search.', 'আপনার অনুসন্ধানে কিছু মেলেনি।')}</p>
              <button onClick={() => reset(() => { setFilter('all'); setYear('all'); setQuery(''); })} className="mt-3 font-sans text-[13px] font-semibold" style={{ color: FJ.brand }}>
              ...
              {slice.map((p) => (
                <article key={p.id} className="card-lift group flex flex-col overflow-hidden rounded-[16px] transition-shadow hover:shadow-lg" style={{ background: FJ.paper, border: `1px solid ${FJ.rule}` }}>
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    ...
                    <h3 className="mt-2 font-serif text-[17px] font-bold leading-snug tracking-tight text-stone-900 group-hover:text-amber-800 transition-colors">{p.title}</h3>
                    <p className="mt-2 line-clamp-2 font-sans text-[13.5px] leading-relaxed text-stone-600">{excerpt(p.content)}…</p>
                    <Link to={`/events/${p.slug || p.id}`}
                      className="mt-4 inline-flex items-center gap-1.5 self-start rounded-full px-4 py-2 font-sans text-[12.5px] font-semibold transition-all duration-200 group-hover:gap-2.5"
